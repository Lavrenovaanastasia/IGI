...
from .models import Cars, Promocode, Sale
from django.contrib.auth.models import User
from django.contrib.auth import get_user_model
from sto.views import *
...
class UserSaleTest(TestCase):
    def setUp(self):
        ...
        self.car = Cars.objects.create(name="Test Car 1", cost=100, categories=self.category, suppliers = self.supplier)
        self.department = Department.objects.create(no = 5, address = "test", close = "13:15:24.548638", open = "13:15:24.548638")
        self.depmed = DepartmentCar.objects.create(quantity = 3, department = self.department, car = self.car)
        self.promocode = Promocode.objects.create(code='MED103', discount=10)

    def test_get_auth_client_pribor_exists(self):
        self.client.force_login(self.user_client)
        response = self.client.get(reverse('create_order', kwargs={'pk': self.car.pk}))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'order_create_form.html')

    def test_get_auth_client_pribor_nonexist(self):
        self.client.force_login(self.user_client)
    ...
    def test_post_auth_client_invalid_data(self):
        ...
        response = self.client.post(reverse('create_order', kwargs={'pk': self.car.pk}), data)
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Выбрать другой филиал или ожидайте завоза деталей")
    ...
    def test_post_auth_client_valid_data(self):
        ...
        response = self.client.post(reverse('create_order', kwargs={'pk': self.car.pk}), data)
        self.assertEqual(response.status_code, 302) 
        self.assertTrue(Sale.objects.filter(user=self.user_client, car=self.car).exists())
...
class SpecificOrderViewTest(TestCase):
    def setUp(self):
        ...
        self.depmed = DepartmentCar.objects.create(quantity = 3, department = self.department, car = self.car)
        self.promocode = Promocode.objects.create(code='MED103', discount=10)
        self.order = Sale.objects.create(department = self.department, promocode = self.promocode, user = self.user_client, car = self.car, quantity = 3, price = 15, price_prom = 15)
...
class CarsListViewTests(TestCase):
    ...
    def setUp(self):
        ...
        self.category = Categories.objects.create(name="Test Category")
        self.supplier = Supplier.objects.create(name="Test Supplier",  address="Test Address", phone="+375(29)1234567")
        self.car1 = Cars.objects.create(name="Test Car 1", cost=100, categories=self.category, suppliers = self.supplier)
        self.car2 = Cars.objects.create(name="Test Car 2", cost=200, categories=self.category, suppliers = self.supplier)

    def test_view_with_no_search_query(self):
        response = self.client.get(self.url)
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Test Car 1")
        self.assertContains(response, "Test Car 2")
        self.assertNotContains(response, "Услуга отсутствует")

    def test_view_with_search_query_found(self):
        response = self.client.get(self.url, {'search': 'Test Car 1'})
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Test Car 1")
        self.assertNotContains(response, "Test Car 2")
        self.assertNotContains(response, "Услуга отсутствует")

    def test_view_with_search_query_not_found(self):
        response = self.client.get(self.url, {'search': 'Nonexistent Car'})
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Услуга отсутствует")
        self.assertNotContains(response, "Test Car 1")
        self.assertNotContains(response, "Test Car 2")

...
def car(request):
    if request.user.is_authenticated and request.user.is_superuser: 
        pribors = Cars.objects.all().order_by('name')
        
        pribor_orders = Sale.objects.values('car__name').annotate(order_count=Count('car__name')).order_by('-order_count')
        most_popular = pribor_orders.first()

        return render(request, 'pribor_stat.html', {'pribors': pribors,
...
def sales(request):
    if request.user.is_authenticated and request.user.is_superuser: 
        ...
        return render(request, 'sale_Рstat.html', {'general_sales': general_sales,
...
def department_revenue_chart(request):
    if request.user.is_authenticated and request.user.is_superuser: 
        ...
        plt.title(f'Преимущество в {current_year} году')
